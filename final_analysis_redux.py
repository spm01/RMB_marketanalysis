...
ctrl_vars_m1 = ['VIX', 'DXY_d', 'US3M_d']
ctrl_vars_m2 = ['VIX', 'DXY_d']
print("Controls going into models:", ctrl_vars_m1, ctrl_vars_m2)

#model 1: CNY-CNH levels
#ai assist for AR-GARCH modeling choice
m1_vars = ['cny_cnh', 'ann_step', 'imp_step'] + ctrl_vars_m1
m1_data = df[m1_vars].dropna()

m1 = arch_model(
    m1_data['cny_cnh'],
    x=m1_data[['ann_step', 'imp_step'] + ctrl_vars_m1],
    mean='ARX', lags=1,
    vol='GARCH', p=1, q=1,
    dist='t'
)
...
m2_vars = ['shibor_hibor_d', 'ann_imp', 'imp_imp'] + ctrl_vars_m2
m2_data = df[m2_vars].dropna()

m2 = arch_model(
    m2_data['shibor_hibor_d'],
    x=m2_data[['ann_imp', 'imp_imp'] + ctrl_vars_m2],
    mean='ARX', lags=1,
    vol='GARCH', p=1, q=1,
    dist='t'
)
